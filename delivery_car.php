...
if (isset($_POST['upload_images'])) {
    ...
    foreach ($image_labels as $index => $label) {
        ...
        if (isset($_FILES[$file_key]) && $_FILES[$file_key]['error'] == 0) {
            ...
            $new_file_name = $reservation_id . '_' . str_replace(' ', '_', strtolower($label)) . '.' . $extension;
    ...
    $python_path = "add your pathon path";  
    $script_path = "add your path \\scan_damage.py";  
...
$update_status_sql = "UPDATE reservations SET status = 'Delivered' WHERE car_id = ?";
...
if ($stmt_update_status->execute()) {
    ...
    // قراءة المحتوى من الحقول
    $damage_before = $_POST['note'] ?? "";
    $notes = $_POST['notes'] ?? ""; // قراءة الحقل الجديد
    
    $created_at = date('Y-m-d H:i:s'); // تاريخ الإضافة
    $damage_count_before = count($uploaded_images); // عدد الصور

    // تخزين الصور في متغيرات
    $front_photo_before = $uploaded_images['Front View'] ?? null;
    $right_photo_before = $uploaded_images['Right Side View'] ?? null;
    $back_photo_before = $uploaded_images['Back View'] ?? null;
    $left_photo_before = $uploaded_images['Left Side View'] ?? null;
//  حفظ البيانات في جدول report
$sql = "INSERT INTO report (
    reservation_id, current_mileage, created_at, damage_before, 
    damage_count_before, 
    front_photo_before, right_photo_before, back_photo_before, left_photo_before,
    note
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
$stmt = $conn->prepare($sql);
...
if ($stmt->execute()) {
echo "<div class='alert alert-success'> Report saved successfully!</div>";

//  حفظ الملاحظة في جدول car_notes أيضًا
$sql_car_notes = "INSERT INTO car_notes (car_id, note, created_at) VALUES (?, ?, ?)";
$stmt_car_notes = $conn->prepare($sql_car_notes);

if ($stmt_car_notes) {
$stmt_car_notes->bind_param('iss', $car_id, $notes, $created_at);
if ($stmt_car_notes->execute()) {
    echo "<div class='alert alert-success'> Note saved successfully in car_notes!</div>";
} else {
    echo "<div class='alert alert-danger'> Error saving note in car_notes: " . $stmt_car_notes->error . "</div>";
}
$stmt_car_notes->close();
} else {
echo "<div class='alert alert-danger'> Error preparing car_notes statement: " . $conn->error . "</div>";
}
} else {
...
<body class="container mt-5">
    ...
    <!--  نموذج رفع الصور -->
    <form action="delivery_car.php?reservation_id=<?php echo $reservation_id; ?>" method="POST" enctype="multipart/form-data" class="row g-3">
...
<script>
    ...
    <?php if ($show_note_and_submit): ?>
        <form action="delivery_car.php?reservation_id=<?php echo $reservation_id; ?>" method="POST" class="row g-3 mt-4">
        <div class="col-md-6">
